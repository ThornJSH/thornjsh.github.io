...
# Each file gets a full <html><head><body> wrapper so Google can crawl them independently

$baseUrl = "https://thornjsh.github.io"

# Page metadata: filename -> [title, description]
$pages = @{
...
foreach ($filename in $pages.Keys) {
    ...
    # Skip if already converted (has <!DOCTYPE html>)
    if ($content -match '<!DOCTYPE html>') {
        Write-Host "SKIP: $filename (already converted)" -ForegroundColor Cyan
        continue
    }

    $title = $pages[$filename][0]
    $description = $pages[$filename][1]
    $canonicalUrl = "$baseUrl/$filename"
    $fullTitle = "$title : Smart Welfare Tech"
...
"@
    ...
    Set-Content -Path $filePath -Value $wrapper -Encoding UTF8 -NoNewline
    $convertedCount++
    Write-Host "OK: $filename -> $title" -ForegroundColor Green
}

Write-Host ""
Write-Host "=== Conversion complete: $convertedCount files converted ===" -ForegroundColor Magenta
